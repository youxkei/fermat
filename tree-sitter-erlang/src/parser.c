#include <tree_sitter/parser.h>

#if defined(__GNUC__) || defined(__clang__)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wmissing-field-initializers"
#endif

#define LANGUAGE_VERSION 12
#define STATE_COUNT 104
#define LARGE_STATE_COUNT 2
#define SYMBOL_COUNT 83
#define ALIAS_COUNT 1
#define TOKEN_COUNT 36
#define EXTERNAL_TOKEN_COUNT 0
#define FIELD_COUNT 0
#define MAX_ALIAS_SEQUENCE_LENGTH 5

enum {
  sym_atom = 1,
  anon_sym_DOT = 2,
  anon_sym_DASH = 3,
  anon_sym_module = 4,
  anon_sym_LPAREN = 5,
  anon_sym_RPAREN = 6,
  anon_sym_export = 7,
  anon_sym_LBRACK = 8,
  anon_sym_COMMA = 9,
  anon_sym_RBRACK = 10,
  anon_sym_SLASH = 11,
  anon_sym_SEMI = 12,
  anon_sym_DASH_GT = 13,
  anon_sym_when = 14,
  anon_sym_catch = 15,
  anon_sym_EQ = 16,
  anon_sym_BANG = 17,
  anon_sym_PLUS = 18,
  anon_sym_bor = 19,
  anon_sym_bxor = 20,
  anon_sym_bsl = 21,
  anon_sym_bsr = 22,
  anon_sym_or = 23,
  anon_sym_xor = 24,
  anon_sym_COLON = 25,
  sym_variable = 26,
  sym_integer = 27,
  anon_sym_DQUOTE = 28,
  aux_sym_string_token1 = 29,
  sym__escape_sequence = 30,
  sym_comment = 31,
  sym_line_comment = 32,
  sym__newline = 33,
  sym_multiple_newlines = 34,
  sym__spaces = 35,
  sym_source_file = 36,
  sym_form = 37,
  sym__attribute = 38,
  sym_module_attribute = 39,
  sym_export_attribute = 40,
  sym_export_attribute_block = 41,
  sym_export_attribute_mfas = 42,
  sym_export_attribute_mfa = 43,
  sym__function_or_macro = 44,
  sym_function_clauses_trailing_semicolon = 45,
  sym_function_clauses = 46,
  sym_function_clause_block = 47,
  sym_function_clause_open = 48,
  sym_pat_argument_list = 49,
  sym_clause_guard = 50,
  sym_function_clause_exprs_trailing_comma = 51,
  sym_exprs = 52,
  sym_catch_expr = 53,
  sym_catch_op = 54,
  sym_equal_exclam_expr = 55,
  sym_equal_op = 56,
  sym_exclam_op = 57,
  sym_orelse_expr = 58,
  sym_andalso_expr = 59,
  sym_comp_expr = 60,
  sym_list_expr = 61,
  sym_add_expr = 62,
  sym_add_op = 63,
  sym_mult_expr = 64,
  sym_prefix_expr = 65,
  sym__other_expr = 66,
  sym_remote_expr = 67,
  sym_function_call_block = 68,
  sym_function_call_open = 69,
  sym__primary_expr = 70,
  sym_list_block = 71,
  sym_list_elements = 72,
  sym__atomic = 73,
  sym_strings = 74,
  sym_string = 75,
  aux_sym_source_file_repeat1 = 76,
  aux_sym_export_attribute_mfas_repeat1 = 77,
  aux_sym_function_clauses_repeat1 = 78,
  aux_sym_exprs_repeat1 = 79,
  aux_sym_add_expr_repeat1 = 80,
  aux_sym_strings_repeat1 = 81,
  aux_sym_string_repeat1 = 82,
  anon_alias_sym_function_clause_exprs = 83,
};

static const char *ts_symbol_names[] = {
  [ts_builtin_sym_end] = "end",
  [sym_atom] = "atom",
  [anon_sym_DOT] = ".",
  [anon_sym_DASH] = "-",
  [anon_sym_module] = "module",
  [anon_sym_LPAREN] = "(",
  [anon_sym_RPAREN] = ")",
  [anon_sym_export] = "export",
  [anon_sym_LBRACK] = "[",
  [anon_sym_COMMA] = ",",
  [anon_sym_RBRACK] = "]",
  [anon_sym_SLASH] = "/",
  [anon_sym_SEMI] = ";",
  [anon_sym_DASH_GT] = "->",
  [anon_sym_when] = "when",
  [anon_sym_catch] = "catch",
  [anon_sym_EQ] = "=",
  [anon_sym_BANG] = "!",
  [anon_sym_PLUS] = "+",
  [anon_sym_bor] = "bor",
  [anon_sym_bxor] = "bxor",
  [anon_sym_bsl] = "bsl",
  [anon_sym_bsr] = "bsr",
  [anon_sym_or] = "or",
  [anon_sym_xor] = "xor",
  [anon_sym_COLON] = ":",
  [sym_variable] = "variable",
  [sym_integer] = "integer",
  [anon_sym_DQUOTE] = "\"",
  [aux_sym_string_token1] = "string_token1",
  [sym__escape_sequence] = "_escape_sequence",
  [sym_comment] = "comment",
  [sym_line_comment] = "line_comment",
  [sym__newline] = "_newline",
  [sym_multiple_newlines] = "multiple_newlines",
  [sym__spaces] = "_spaces",
  [sym_source_file] = "source_file",
  [sym_form] = "form",
  [sym__attribute] = "_attribute",
  [sym_module_attribute] = "module_attribute",
  [sym_export_attribute] = "export_attribute",
  [sym_export_attribute_block] = "export_attribute_block",
  [sym_export_attribute_mfas] = "export_attribute_mfas",
  [sym_export_attribute_mfa] = "export_attribute_mfa",
  [sym__function_or_macro] = "_function_or_macro",
  [sym_function_clauses_trailing_semicolon] = "function_clauses_trailing_semicolon",
  [sym_function_clauses] = "function_clauses",
  [sym_function_clause_block] = "function_clause_block",
  [sym_function_clause_open] = "function_clause_open",
  [sym_pat_argument_list] = "pat_argument_list",
  [sym_clause_guard] = "clause_guard",
  [sym_function_clause_exprs_trailing_comma] = "function_clause_exprs_trailing_comma",
  [sym_exprs] = "exprs",
  [sym_catch_expr] = "catch_expr",
  [sym_catch_op] = "catch_op",
  [sym_equal_exclam_expr] = "equal_exclam_expr",
  [sym_equal_op] = "equal_op",
  [sym_exclam_op] = "exclam_op",
  [sym_orelse_expr] = "orelse_expr",
  [sym_andalso_expr] = "andalso_expr",
  [sym_comp_expr] = "comp_expr",
  [sym_list_expr] = "list_expr",
  [sym_add_expr] = "add_expr",
  [sym_add_op] = "add_op",
  [sym_mult_expr] = "mult_expr",
  [sym_prefix_expr] = "prefix_expr",
  [sym__other_expr] = "_other_expr",
  [sym_remote_expr] = "remote_expr",
  [sym_function_call_block] = "function_call_block",
  [sym_function_call_open] = "function_call_open",
  [sym__primary_expr] = "_primary_expr",
  [sym_list_block] = "list_block",
  [sym_list_elements] = "list_elements",
  [sym__atomic] = "_atomic",
  [sym_strings] = "strings",
  [sym_string] = "string",
  [aux_sym_source_file_repeat1] = "source_file_repeat1",
  [aux_sym_export_attribute_mfas_repeat1] = "export_attribute_mfas_repeat1",
  [aux_sym_function_clauses_repeat1] = "function_clauses_repeat1",
  [aux_sym_exprs_repeat1] = "exprs_repeat1",
  [aux_sym_add_expr_repeat1] = "add_expr_repeat1",
  [aux_sym_strings_repeat1] = "strings_repeat1",
  [aux_sym_string_repeat1] = "string_repeat1",
  [anon_alias_sym_function_clause_exprs] = "function_clause_exprs",
};

static TSSymbol ts_symbol_map[] = {
  [ts_builtin_sym_end] = ts_builtin_sym_end,
  [sym_atom] = sym_atom,
  [anon_sym_DOT] = anon_sym_DOT,
  [anon_sym_DASH] = anon_sym_DASH,
  [anon_sym_module] = anon_sym_module,
  [anon_sym_LPAREN] = anon_sym_LPAREN,
  [anon_sym_RPAREN] = anon_sym_RPAREN,
  [anon_sym_export] = anon_sym_export,
  [anon_sym_LBRACK] = anon_sym_LBRACK,
  [anon_sym_COMMA] = anon_sym_COMMA,
  [anon_sym_RBRACK] = anon_sym_RBRACK,
  [anon_sym_SLASH] = anon_sym_SLASH,
  [anon_sym_SEMI] = anon_sym_SEMI,
  [anon_sym_DASH_GT] = anon_sym_DASH_GT,
  [anon_sym_when] = anon_sym_when,
  [anon_sym_catch] = anon_sym_catch,
  [anon_sym_EQ] = anon_sym_EQ,
  [anon_sym_BANG] = anon_sym_BANG,
  [anon_sym_PLUS] = anon_sym_PLUS,
  [anon_sym_bor] = anon_sym_bor,
  [anon_sym_bxor] = anon_sym_bxor,
  [anon_sym_bsl] = anon_sym_bsl,
  [anon_sym_bsr] = anon_sym_bsr,
  [anon_sym_or] = anon_sym_or,
  [anon_sym_xor] = anon_sym_xor,
  [anon_sym_COLON] = anon_sym_COLON,
  [sym_variable] = sym_variable,
  [sym_integer] = sym_integer,
  [anon_sym_DQUOTE] = anon_sym_DQUOTE,
  [aux_sym_string_token1] = aux_sym_string_token1,
  [sym__escape_sequence] = sym__escape_sequence,
  [sym_comment] = sym_comment,
  [sym_line_comment] = sym_line_comment,
  [sym__newline] = sym__newline,
  [sym_multiple_newlines] = sym_multiple_newlines,
  [sym__spaces] = sym__spaces,
  [sym_source_file] = sym_source_file,
  [sym_form] = sym_form,
  [sym__attribute] = sym__attribute,
  [sym_module_attribute] = sym_module_attribute,
  [sym_export_attribute] = sym_export_attribute,
  [sym_export_attribute_block] = sym_export_attribute_block,
  [sym_export_attribute_mfas] = sym_export_attribute_mfas,
  [sym_export_attribute_mfa] = sym_export_attribute_mfa,
  [sym__function_or_macro] = sym__function_or_macro,
  [sym_function_clauses_trailing_semicolon] = sym_function_clauses_trailing_semicolon,
  [sym_function_clauses] = sym_function_clauses,
  [sym_function_clause_block] = sym_function_clause_block,
  [sym_function_clause_open] = sym_function_clause_open,
  [sym_pat_argument_list] = sym_pat_argument_list,
  [sym_clause_guard] = sym_clause_guard,
  [sym_function_clause_exprs_trailing_comma] = sym_function_clause_exprs_trailing_comma,
  [sym_exprs] = sym_exprs,
  [sym_catch_expr] = sym_catch_expr,
  [sym_catch_op] = sym_catch_op,
  [sym_equal_exclam_expr] = sym_equal_exclam_expr,
  [sym_equal_op] = sym_equal_op,
  [sym_exclam_op] = sym_exclam_op,
  [sym_orelse_expr] = sym_orelse_expr,
  [sym_andalso_expr] = sym_andalso_expr,
  [sym_comp_expr] = sym_comp_expr,
  [sym_list_expr] = sym_list_expr,
  [sym_add_expr] = sym_add_expr,
  [sym_add_op] = sym_add_op,
  [sym_mult_expr] = sym_mult_expr,
  [sym_prefix_expr] = sym_prefix_expr,
  [sym__other_expr] = sym__other_expr,
  [sym_remote_expr] = sym_remote_expr,
  [sym_function_call_block] = sym_function_call_block,
  [sym_function_call_open] = sym_function_call_open,
  [sym__primary_expr] = sym__primary_expr,
  [sym_list_block] = sym_list_block,
  [sym_list_elements] = sym_list_elements,
  [sym__atomic] = sym__atomic,
  [sym_strings] = sym_strings,
  [sym_string] = sym_string,
  [aux_sym_source_file_repeat1] = aux_sym_source_file_repeat1,
  [aux_sym_export_attribute_mfas_repeat1] = aux_sym_export_attribute_mfas_repeat1,
  [aux_sym_function_clauses_repeat1] = aux_sym_function_clauses_repeat1,
  [aux_sym_exprs_repeat1] = aux_sym_exprs_repeat1,
  [aux_sym_add_expr_repeat1] = aux_sym_add_expr_repeat1,
  [aux_sym_strings_repeat1] = aux_sym_strings_repeat1,
  [aux_sym_string_repeat1] = aux_sym_string_repeat1,
  [anon_alias_sym_function_clause_exprs] = anon_alias_sym_function_clause_exprs,
};

static const TSSymbolMetadata ts_symbol_metadata[] = {
  [ts_builtin_sym_end] = {
    .visible = false,
    .named = true,
  },
  [sym_atom] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_DOT] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_DASH] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_module] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_LPAREN] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_RPAREN] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_export] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_LBRACK] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_COMMA] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_RBRACK] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_SLASH] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_SEMI] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_DASH_GT] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_when] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_catch] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_BANG] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_PLUS] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_bor] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_bxor] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_bsl] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_bsr] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_or] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_xor] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_COLON] = {
    .visible = true,
    .named = false,
  },
  [sym_variable] = {
    .visible = true,
    .named = true,
  },
  [sym_integer] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_DQUOTE] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_string_token1] = {
    .visible = false,
    .named = false,
  },
  [sym__escape_sequence] = {
    .visible = false,
    .named = true,
  },
  [sym_comment] = {
    .visible = true,
    .named = true,
  },
  [sym_line_comment] = {
    .visible = true,
    .named = true,
  },
  [sym__newline] = {
    .visible = false,
    .named = true,
  },
  [sym_multiple_newlines] = {
    .visible = true,
    .named = true,
  },
  [sym__spaces] = {
    .visible = false,
    .named = true,
  },
  [sym_source_file] = {
    .visible = true,
    .named = true,
  },
  [sym_form] = {
    .visible = true,
    .named = true,
  },
  [sym__attribute] = {
    .visible = false,
    .named = true,
  },
  [sym_module_attribute] = {
    .visible = true,
    .named = true,
  },
  [sym_export_attribute] = {
    .visible = true,
    .named = true,
  },
  [sym_export_attribute_block] = {
    .visible = true,
    .named = true,
  },
  [sym_export_attribute_mfas] = {
    .visible = true,
    .named = true,
  },
  [sym_export_attribute_mfa] = {
    .visible = true,
    .named = true,
  },
  [sym__function_or_macro] = {
    .visible = false,
    .named = true,
  },
  [sym_function_clauses_trailing_semicolon] = {
    .visible = true,
    .named = true,
  },
  [sym_function_clauses] = {
    .visible = true,
    .named = true,
  },
  [sym_function_clause_block] = {
    .visible = true,
    .named = true,
  },
  [sym_function_clause_open] = {
    .visible = true,
    .named = true,
  },
  [sym_pat_argument_list] = {
    .visible = true,
    .named = true,
  },
  [sym_clause_guard] = {
    .visible = true,
    .named = true,
  },
  [sym_function_clause_exprs_trailing_comma] = {
    .visible = true,
    .named = true,
  },
  [sym_exprs] = {
    .visible = true,
    .named = true,
  },
  [sym_catch_expr] = {
    .visible = true,
    .named = true,
  },
  [sym_catch_op] = {
    .visible = true,
    .named = true,
  },
  [sym_equal_exclam_expr] = {
    .visible = true,
    .named = true,
  },
  [sym_equal_op] = {
    .visible = true,
    .named = true,
  },
  [sym_exclam_op] = {
    .visible = true,
    .named = true,
  },
  [sym_orelse_expr] = {
    .visible = true,
    .named = true,
  },
  [sym_andalso_expr] = {
    .visible = true,
    .named = true,
  },
  [sym_comp_expr] = {
    .visible = true,
    .named = true,
  },
  [sym_list_expr] = {
    .visible = true,
    .named = true,
  },
  [sym_add_expr] = {
    .visible = true,
    .named = true,
  },
  [sym_add_op] = {
    .visible = true,
    .named = true,
  },
  [sym_mult_expr] = {
    .visible = true,
    .named = true,
  },
  [sym_prefix_expr] = {
    .visible = true,
    .named = true,
  },
  [sym__other_expr] = {
    .visible = false,
    .named = true,
  },
  [sym_remote_expr] = {
    .visible = true,
    .named = true,
  },
  [sym_function_call_block] = {
    .visible = true,
    .named = true,
  },
  [sym_function_call_open] = {
    .visible = true,
    .named = true,
  },
  [sym__primary_expr] = {
    .visible = false,
    .named = true,
  },
  [sym_list_block] = {
    .visible = true,
    .named = true,
  },
  [sym_list_elements] = {
    .visible = true,
    .named = true,
  },
  [sym__atomic] = {
    .visible = false,
    .named = true,
  },
  [sym_strings] = {
    .visible = true,
    .named = true,
  },
  [sym_string] = {
    .visible = true,
    .named = true,
  },
  [aux_sym_source_file_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_export_attribute_mfas_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_function_clauses_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_exprs_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_add_expr_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_strings_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_string_repeat1] = {
    .visible = false,
    .named = false,
  },
  [anon_alias_sym_function_clause_exprs] = {
    .visible = true,
    .named = false,
  },
};

static TSSymbol ts_alias_sequences[2][MAX_ALIAS_SEQUENCE_LENGTH] = {
  [0] = {0},
  [1] = {
    [0] = anon_alias_sym_function_clause_exprs,
  },
};

static uint16_t ts_non_terminal_alias_map[] = {
  sym_exprs, 2,
    sym_exprs,
    anon_alias_sym_function_clause_exprs,
  0,
};

static bool ts_lex(TSLexer *lexer, TSStateId state) {
  START_LEXER();
  eof = lexer->eof(lexer);
  switch (state) {
    case 0:
      if (eof) ADVANCE(11);
      if (lookahead == '\n') ADVANCE(34);
      if (lookahead == '!') ADVANCE(41);
      if (lookahead == '"') ADVANCE(33);
      if (lookahead == '%') ADVANCE(36);
      if (lookahead == '\'') ADVANCE(37);
      if (lookahead == '(') ADVANCE(41);
      if (lookahead == ')') ADVANCE(41);
      if (lookahead == '+') ADVANCE(41);
      if (lookahead == ',') ADVANCE(41);
      if (lookahead == '-') ADVANCE(38);
      if (lookahead == '.') ADVANCE(41);
      if (lookahead == '/') ADVANCE(41);
      if (lookahead == ':') ADVANCE(41);
      if (lookahead == ';') ADVANCE(41);
      if (lookahead == '=') ADVANCE(41);
      if (lookahead == '[') ADVANCE(41);
      if (lookahead == '\\') ADVANCE(6);
      if (lookahead == ']') ADVANCE(41);
      if (lookahead == '\t' ||
          lookahead == '\r' ||
          lookahead == ' ') ADVANCE(41);
      if (('A' <= lookahead && lookahead <= 'Z')) ADVANCE(39);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(40);
      if (lookahead != 0) ADVANCE(41);
      END_STATE();
    case 1:
      if (lookahead == '\n') ADVANCE(34);
      if (lookahead == '"') ADVANCE(33);
      if (lookahead == '%') ADVANCE(36);
      if (lookahead == '\\') ADVANCE(6);
      if (lookahead == '\t' ||
          lookahead == '\r' ||
          lookahead == ' ') ADVANCE(41);
      if (lookahead != 0) ADVANCE(41);
      END_STATE();
    case 2:
      if (lookahead == '\n') ADVANCE(47);
      if (lookahead == '"') ADVANCE(33);
      if (lookahead == '%') ADVANCE(3);
      if (lookahead == '\'') ADVANCE(4);
      if (lookahead == ')') ADVANCE(15);
      if (lookahead == ',') ADVANCE(17);
      if (lookahead == '-') ADVANCE(5);
      if (lookahead == '[') ADVANCE(16);
      if (lookahead == ']') ADVANCE(18);
      if (lookahead == '\t' ||
          lookahead == '\r' ||
          lookahead == ' ') ADVANCE(49);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(30);
      if (('A' <= lookahead && lookahead <= 'Z')) ADVANCE(26);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(29);
      END_STATE();
    case 3:
      if (lookahead == '%') ADVANCE(46);
      if (lookahead != 0) ADVANCE(45);
      END_STATE();
    case 4:
      if (lookahead == '\'') ADVANCE(27);
      if (lookahead == '\\') ADVANCE(9);
      if (lookahead != 0) ADVANCE(4);
      END_STATE();
    case 5:
      if (lookahead == '>') ADVANCE(21);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(30);
      END_STATE();
    case 6:
      if (lookahead == 'x') ADVANCE(7);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(44);
      if (lookahead != 0 &&
          lookahead != 'U' &&
          lookahead != 'u') ADVANCE(42);
      END_STATE();
    case 7:
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'F') ||
          ('a' <= lookahead && lookahead <= 'f')) ADVANCE(42);
      END_STATE();
    case 8:
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'F') ||
          ('a' <= lookahead && lookahead <= 'f')) ADVANCE(32);
      END_STATE();
    case 9:
      if (lookahead != 0 &&
          lookahead != '\'' &&
          lookahead != '\\') ADVANCE(4);
      if (lookahead == '\'') ADVANCE(28);
      if (lookahead == '\\') ADVANCE(9);
      END_STATE();
    case 10:
      if (eof) ADVANCE(11);
      if (lookahead == '\n') ADVANCE(47);
      if (lookahead == '!') ADVANCE(23);
      if (lookahead == '"') ADVANCE(33);
      if (lookahead == '%') ADVANCE(3);
      if (lookahead == '\'') ADVANCE(4);
      if (lookahead == '(') ADVANCE(14);
      if (lookahead == ')') ADVANCE(15);
      if (lookahead == '+') ADVANCE(24);
      if (lookahead == ',') ADVANCE(17);
      if (lookahead == '-') ADVANCE(13);
      if (lookahead == '.') ADVANCE(12);
      if (lookahead == '/') ADVANCE(19);
      if (lookahead == ':') ADVANCE(25);
      if (lookahead == ';') ADVANCE(20);
      if (lookahead == '=') ADVANCE(22);
      if (lookahead == '[') ADVANCE(16);
      if (lookahead == ']') ADVANCE(18);
      if (lookahead == '\t' ||
          lookahead == '\r' ||
          lookahead == ' ') ADVANCE(49);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(29);
      END_STATE();
    case 11:
      ACCEPT_TOKEN(ts_builtin_sym_end);
      END_STATE();
    case 12:
      ACCEPT_TOKEN(anon_sym_DOT);
      END_STATE();
    case 13:
      ACCEPT_TOKEN(anon_sym_DASH);
      END_STATE();
    case 14:
      ACCEPT_TOKEN(anon_sym_LPAREN);
      END_STATE();
    case 15:
      ACCEPT_TOKEN(anon_sym_RPAREN);
      END_STATE();
    case 16:
      ACCEPT_TOKEN(anon_sym_LBRACK);
      END_STATE();
    case 17:
      ACCEPT_TOKEN(anon_sym_COMMA);
      END_STATE();
    case 18:
      ACCEPT_TOKEN(anon_sym_RBRACK);
      END_STATE();
    case 19:
      ACCEPT_TOKEN(anon_sym_SLASH);
      END_STATE();
    case 20:
      ACCEPT_TOKEN(anon_sym_SEMI);
      END_STATE();
    case 21:
      ACCEPT_TOKEN(anon_sym_DASH_GT);
      END_STATE();
    case 22:
      ACCEPT_TOKEN(anon_sym_EQ);
      END_STATE();
    case 23:
      ACCEPT_TOKEN(anon_sym_BANG);
      END_STATE();
    case 24:
      ACCEPT_TOKEN(anon_sym_PLUS);
      END_STATE();
    case 25:
      ACCEPT_TOKEN(anon_sym_COLON);
      END_STATE();
    case 26:
      ACCEPT_TOKEN(sym_variable);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(26);
      END_STATE();
    case 27:
      ACCEPT_TOKEN(sym_atom);
      END_STATE();
    case 28:
      ACCEPT_TOKEN(sym_atom);
      if (lookahead == '\'') ADVANCE(27);
      if (lookahead == '\\') ADVANCE(9);
      if (lookahead != 0) ADVANCE(4);
      END_STATE();
    case 29:
      ACCEPT_TOKEN(sym_atom);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('@' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(29);
      END_STATE();
    case 30:
      ACCEPT_TOKEN(sym_integer);
      if (lookahead == '#') ADVANCE(8);
      if (lookahead == '_') ADVANCE(31);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(30);
      END_STATE();
    case 31:
      ACCEPT_TOKEN(sym_integer);
      if (('0' <= lookahead && lookahead <= '9') ||
          lookahead == '_') ADVANCE(31);
      END_STATE();
    case 32:
      ACCEPT_TOKEN(sym_integer);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'F') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'f')) ADVANCE(32);
      END_STATE();
    case 33:
      ACCEPT_TOKEN(anon_sym_DQUOTE);
      END_STATE();
    case 34:
      ACCEPT_TOKEN(aux_sym_string_token1);
      if (lookahead == '\n') ADVANCE(34);
      if (lookahead != 0 &&
          lookahead != '"' &&
          lookahead != '\\') ADVANCE(41);
      END_STATE();
    case 35:
      ACCEPT_TOKEN(aux_sym_string_token1);
      if (lookahead == '\n') ADVANCE(41);
      if (lookahead != 0 &&
          lookahead != '"' &&
          lookahead != '\\') ADVANCE(35);
      END_STATE();
    case 36:
      ACCEPT_TOKEN(aux_sym_string_token1);
      if (lookahead == '%') ADVANCE(35);
      if (lookahead != 0 &&
          lookahead != '"' &&
          lookahead != '\\') ADVANCE(35);
      END_STATE();
    case 37:
      ACCEPT_TOKEN(aux_sym_string_token1);
      if (lookahead == '\'') ADVANCE(41);
      if (lookahead != 0 &&
          lookahead != '"' &&
          lookahead != '\\') ADVANCE(37);
      END_STATE();
    case 38:
      ACCEPT_TOKEN(aux_sym_string_token1);
      if (lookahead == '>') ADVANCE(41);
      if (lookahead != 0 &&
          lookahead != '"' &&
          lookahead != '\\') ADVANCE(41);
      END_STATE();
    case 39:
      ACCEPT_TOKEN(aux_sym_string_token1);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(39);
      if (lookahead != 0 &&
          lookahead != '"' &&
          lookahead != '\\') ADVANCE(41);
      END_STATE();
    case 40:
      ACCEPT_TOKEN(aux_sym_string_token1);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('@' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(40);
      if (lookahead != 0 &&
          lookahead != '"' &&
          lookahead != '\\') ADVANCE(41);
      END_STATE();
    case 41:
      ACCEPT_TOKEN(aux_sym_string_token1);
      if (lookahead != 0 &&
          lookahead != '"' &&
          lookahead != '\\') ADVANCE(41);
      END_STATE();
    case 42:
      ACCEPT_TOKEN(sym__escape_sequence);
      END_STATE();
    case 43:
      ACCEPT_TOKEN(sym__escape_sequence);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(42);
      END_STATE();
    case 44:
      ACCEPT_TOKEN(sym__escape_sequence);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(43);
      END_STATE();
    case 45:
      ACCEPT_TOKEN(sym_comment);
      if (lookahead != 0 &&
          lookahead != '\n') ADVANCE(45);
      END_STATE();
    case 46:
      ACCEPT_TOKEN(sym_line_comment);
      if (lookahead != 0 &&
          lookahead != '\n') ADVANCE(46);
      END_STATE();
    case 47:
      ACCEPT_TOKEN(sym__newline);
      if (lookahead == '\n') ADVANCE(48);
      END_STATE();
    case 48:
      ACCEPT_TOKEN(sym_multiple_newlines);
      if (lookahead == '\n') ADVANCE(48);
      END_STATE();
    case 49:
      ACCEPT_TOKEN(sym__spaces);
      END_STATE();
    default:
      return false;
  }
}

static bool ts_lex_keywords(TSLexer *lexer, TSStateId state) {
  START_LEXER();
  eof = lexer->eof(lexer);
  switch (state) {
    case 0:
      if (lookahead == 'b') ADVANCE(1);
      if (lookahead == 'c') ADVANCE(2);
      if (lookahead == 'e') ADVANCE(3);
      if (lookahead == 'm') ADVANCE(4);
      if (lookahead == 'o') ADVANCE(5);
      if (lookahead == 'w') ADVANCE(6);
      if (lookahead == 'x') ADVANCE(7);
      END_STATE();
    case 1:
      if (lookahead == 'o') ADVANCE(8);
      if (lookahead == 's') ADVANCE(9);
      if (lookahead == 'x') ADVANCE(10);
      END_STATE();
    case 2:
      if (lookahead == 'a') ADVANCE(11);
      END_STATE();
    case 3:
      if (lookahead == 'x') ADVANCE(12);
      END_STATE();
    case 4:
      if (lookahead == 'o') ADVANCE(13);
      END_STATE();
    case 5:
      if (lookahead == 'r') ADVANCE(14);
      END_STATE();
    case 6:
      if (lookahead == 'h') ADVANCE(15);
      END_STATE();
    case 7:
      if (lookahead == 'o') ADVANCE(16);
      END_STATE();
    case 8:
      if (lookahead == 'r') ADVANCE(17);
      END_STATE();
    case 9:
      if (lookahead == 'l') ADVANCE(18);
      if (lookahead == 'r') ADVANCE(19);
      END_STATE();
    case 10:
      if (lookahead == 'o') ADVANCE(20);
      END_STATE();
    case 11:
      if (lookahead == 't') ADVANCE(21);
      END_STATE();
    case 12:
      if (lookahead == 'p') ADVANCE(22);
      END_STATE();
    case 13:
      if (lookahead == 'd') ADVANCE(23);
      END_STATE();
    case 14:
      ACCEPT_TOKEN(anon_sym_or);
      END_STATE();
    case 15:
      if (lookahead == 'e') ADVANCE(24);
      END_STATE();
    case 16:
      if (lookahead == 'r') ADVANCE(25);
      END_STATE();
    case 17:
      ACCEPT_TOKEN(anon_sym_bor);
      END_STATE();
    case 18:
      ACCEPT_TOKEN(anon_sym_bsl);
      END_STATE();
    case 19:
      ACCEPT_TOKEN(anon_sym_bsr);
      END_STATE();
    case 20:
      if (lookahead == 'r') ADVANCE(26);
      END_STATE();
    case 21:
      if (lookahead == 'c') ADVANCE(27);
      END_STATE();
    case 22:
      if (lookahead == 'o') ADVANCE(28);
      END_STATE();
    case 23:
      if (lookahead == 'u') ADVANCE(29);
      END_STATE();
    case 24:
      if (lookahead == 'n') ADVANCE(30);
      END_STATE();
    case 25:
      ACCEPT_TOKEN(anon_sym_xor);
      END_STATE();
    case 26:
      ACCEPT_TOKEN(anon_sym_bxor);
      END_STATE();
    case 27:
      if (lookahead == 'h') ADVANCE(31);
      END_STATE();
    case 28:
      if (lookahead == 'r') ADVANCE(32);
      END_STATE();
    case 29:
      if (lookahead == 'l') ADVANCE(33);
      END_STATE();
    case 30:
      ACCEPT_TOKEN(anon_sym_when);
      END_STATE();
    case 31:
      ACCEPT_TOKEN(anon_sym_catch);
      END_STATE();
    case 32:
      if (lookahead == 't') ADVANCE(34);
      END_STATE();
    case 33:
      if (lookahead == 'e') ADVANCE(35);
      END_STATE();
    case 34:
      ACCEPT_TOKEN(anon_sym_export);
      END_STATE();
    case 35:
      ACCEPT_TOKEN(anon_sym_module);
      END_STATE();
    default:
      return false;
  }
}

static TSLexMode ts_lex_modes[STATE_COUNT] = {
  [0] = {.lex_state = 0},
  [1] = {.lex_state = 10},
  [2] = {.lex_state = 2},
  [3] = {.lex_state = 2},
  [4] = {.lex_state = 2},
  [5] = {.lex_state = 2},
  [6] = {.lex_state = 2},
  [7] = {.lex_state = 2},
  [8] = {.lex_state = 10},
  [9] = {.lex_state = 10},
  [10] = {.lex_state = 10},
  [11] = {.lex_state = 10},
  [12] = {.lex_state = 10},
  [13] = {.lex_state = 10},
  [14] = {.lex_state = 10},
  [15] = {.lex_state = 2},
  [16] = {.lex_state = 10},
  [17] = {.lex_state = 10},
  [18] = {.lex_state = 10},
  [19] = {.lex_state = 10},
  [20] = {.lex_state = 10},
  [21] = {.lex_state = 10},
  [22] = {.lex_state = 10},
  [23] = {.lex_state = 10},
  [24] = {.lex_state = 10},
  [25] = {.lex_state = 10},
  [26] = {.lex_state = 10},
  [27] = {.lex_state = 10},
  [28] = {.lex_state = 10},
  [29] = {.lex_state = 10},
  [30] = {.lex_state = 2},
  [31] = {.lex_state = 10},
  [32] = {.lex_state = 2},
  [33] = {.lex_state = 10},
  [34] = {.lex_state = 10},
  [35] = {.lex_state = 10},
  [36] = {.lex_state = 10},
  [37] = {.lex_state = 2},
  [38] = {.lex_state = 10},
  [39] = {.lex_state = 2},
  [40] = {.lex_state = 10},
  [41] = {.lex_state = 10},
  [42] = {.lex_state = 10},
  [43] = {.lex_state = 10},
  [44] = {.lex_state = 2},
  [45] = {.lex_state = 2},
  [46] = {.lex_state = 10},
  [47] = {.lex_state = 10},
  [48] = {.lex_state = 2},
  [49] = {.lex_state = 2},
  [50] = {.lex_state = 10},
  [51] = {.lex_state = 1},
  [52] = {.lex_state = 1},
  [53] = {.lex_state = 1},
  [54] = {.lex_state = 10},
  [55] = {.lex_state = 10},
  [56] = {.lex_state = 10},
  [57] = {.lex_state = 10},
  [58] = {.lex_state = 10},
  [59] = {.lex_state = 10},
  [60] = {.lex_state = 2},
  [61] = {.lex_state = 10},
  [62] = {.lex_state = 10},
  [63] = {.lex_state = 10},
  [64] = {.lex_state = 10},
  [65] = {.lex_state = 10},
  [66] = {.lex_state = 10},
  [67] = {.lex_state = 10},
  [68] = {.lex_state = 10},
  [69] = {.lex_state = 10},
  [70] = {.lex_state = 10},
  [71] = {.lex_state = 10},
  [72] = {.lex_state = 10},
  [73] = {.lex_state = 10},
  [74] = {.lex_state = 10},
  [75] = {.lex_state = 2},
  [76] = {.lex_state = 10},
  [77] = {.lex_state = 10},
  [78] = {.lex_state = 10},
  [79] = {.lex_state = 10},
  [80] = {.lex_state = 10},
  [81] = {.lex_state = 10},
  [82] = {.lex_state = 10},
  [83] = {.lex_state = 10},
  [84] = {.lex_state = 10},
  [85] = {.lex_state = 10},
  [86] = {.lex_state = 10},
  [87] = {.lex_state = 10},
  [88] = {.lex_state = 10},
  [89] = {.lex_state = 10},
  [90] = {.lex_state = 10},
  [91] = {.lex_state = 2},
  [92] = {.lex_state = 10},
  [93] = {.lex_state = 10},
  [94] = {.lex_state = 10},
  [95] = {.lex_state = 10},
  [96] = {.lex_state = 2},
  [97] = {.lex_state = 10},
  [98] = {.lex_state = 10},
  [99] = {.lex_state = 10},
  [100] = {.lex_state = 10},
  [101] = {.lex_state = 10},
  [102] = {.lex_state = 10},
  [103] = {.lex_state = 2},
};

static uint16_t ts_parse_table[LARGE_STATE_COUNT][SYMBOL_COUNT] = {
  [0] = {
    [ts_builtin_sym_end] = ACTIONS(1),
    [sym_atom] = ACTIONS(1),
    [anon_sym_DOT] = ACTIONS(1),
    [anon_sym_DASH] = ACTIONS(1),
    [anon_sym_module] = ACTIONS(1),
    [anon_sym_LPAREN] = ACTIONS(1),
    [anon_sym_RPAREN] = ACTIONS(1),
    [anon_sym_export] = ACTIONS(1),
    [anon_sym_LBRACK] = ACTIONS(1),
    [anon_sym_COMMA] = ACTIONS(1),
    [anon_sym_RBRACK] = ACTIONS(1),
    [anon_sym_SLASH] = ACTIONS(1),
    [anon_sym_SEMI] = ACTIONS(1),
    [anon_sym_DASH_GT] = ACTIONS(1),
    [anon_sym_when] = ACTIONS(1),
    [anon_sym_catch] = ACTIONS(1),
    [anon_sym_EQ] = ACTIONS(1),
    [anon_sym_BANG] = ACTIONS(1),
    [anon_sym_PLUS] = ACTIONS(1),
    [anon_sym_bor] = ACTIONS(1),
    [anon_sym_bxor] = ACTIONS(1),
    [anon_sym_bsl] = ACTIONS(1),
    [anon_sym_bsr] = ACTIONS(1),
    [anon_sym_or] = ACTIONS(1),
    [anon_sym_xor] = ACTIONS(1),
    [anon_sym_COLON] = ACTIONS(1),
    [sym_variable] = ACTIONS(1),
    [anon_sym_DQUOTE] = ACTIONS(1),
    [aux_sym_string_token1] = ACTIONS(1),
    [sym__escape_sequence] = ACTIONS(1),
    [sym_comment] = ACTIONS(3),
    [sym_line_comment] = ACTIONS(3),
    [sym__newline] = ACTIONS(3),
    [sym_multiple_newlines] = ACTIONS(3),
    [sym__spaces] = ACTIONS(3),
  },
  [1] = {
    [sym_source_file] = STATE(93),
    [sym_form] = STATE(29),
    [sym__attribute] = STATE(85),
    [sym_module_attribute] = STATE(85),
    [sym_export_attribute] = STATE(85),
    [sym__function_or_macro] = STATE(85),
    [sym_function_clauses_trailing_semicolon] = STATE(85),
    [sym_function_clauses] = STATE(79),
    [sym_function_clause_block] = STATE(57),
    [sym_function_clause_open] = STATE(4),
    [aux_sym_source_file_repeat1] = STATE(29),
    [sym_atom] = ACTIONS(5),
    [anon_sym_DASH] = ACTIONS(7),
    [sym_comment] = ACTIONS(9),
    [sym_line_comment] = ACTIONS(9),
    [sym__newline] = ACTIONS(3),
    [sym_multiple_newlines] = ACTIONS(9),
    [sym__spaces] = ACTIONS(9),
  },
};

static uint16_t ts_small_parse_table[] = {
  [0] = 25,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(11), 1,
      sym_atom,
    ACTIONS(13), 1,
      anon_sym_RPAREN,
    ACTIONS(15), 1,
      anon_sym_LBRACK,
    ACTIONS(17), 1,
      anon_sym_COMMA,
    ACTIONS(19), 1,
      anon_sym_catch,
    ACTIONS(23), 1,
      anon_sym_DQUOTE,
    STATE(2), 1,
      sym_function_call_open,
    STATE(6), 1,
      sym_catch_op,
    STATE(17), 1,
      sym_mult_expr,
    STATE(20), 1,
      sym_remote_expr,
    STATE(26), 1,
      sym_prefix_expr,
    STATE(31), 1,
      sym_orelse_expr,
    STATE(33), 1,
      sym_add_expr,
    STATE(34), 1,
      sym_andalso_expr,
    STATE(35), 1,
      sym_comp_expr,
    STATE(36), 1,
      sym_list_expr,
    STATE(41), 1,
      sym_equal_exclam_expr,
    STATE(50), 1,
      sym_catch_expr,
    STATE(66), 1,
      sym_exprs,
    ACTIONS(21), 2,
      sym_variable,
      sym_integer,
    STATE(8), 2,
      sym_string,
      aux_sym_strings_repeat1,
    STATE(27), 2,
      sym__other_expr,
      sym_function_call_block,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    STATE(21), 4,
      sym__primary_expr,
      sym_list_block,
      sym__atomic,
      sym_strings,
  [85] = 25,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(11), 1,
      sym_atom,
    ACTIONS(15), 1,
      anon_sym_LBRACK,
    ACTIONS(19), 1,
      anon_sym_catch,
    ACTIONS(23), 1,
      anon_sym_DQUOTE,
    ACTIONS(25), 1,
      anon_sym_COMMA,
    ACTIONS(27), 1,
      anon_sym_RBRACK,
    STATE(2), 1,
      sym_function_call_open,
    STATE(6), 1,
      sym_catch_op,
    STATE(17), 1,
      sym_mult_expr,
    STATE(20), 1,
      sym_remote_expr,
    STATE(26), 1,
      sym_prefix_expr,
    STATE(31), 1,
      sym_orelse_expr,
    STATE(33), 1,
      sym_add_expr,
    STATE(34), 1,
      sym_andalso_expr,
    STATE(35), 1,
      sym_comp_expr,
    STATE(36), 1,
      sym_list_expr,
    STATE(41), 1,
      sym_equal_exclam_expr,
    STATE(56), 1,
      sym_catch_expr,
    STATE(78), 1,
      sym_list_elements,
    ACTIONS(21), 2,
      sym_variable,
      sym_integer,
    STATE(8), 2,
      sym_string,
      aux_sym_strings_repeat1,
    STATE(27), 2,
      sym__other_expr,
      sym_function_call_block,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    STATE(21), 4,
      sym__primary_expr,
      sym_list_block,
      sym__atomic,
      sym_strings,
  [170] = 24,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(11), 1,
      sym_atom,
    ACTIONS(15), 1,
      anon_sym_LBRACK,
    ACTIONS(19), 1,
      anon_sym_catch,
    ACTIONS(23), 1,
      anon_sym_DQUOTE,
    STATE(2), 1,
      sym_function_call_open,
    STATE(6), 1,
      sym_catch_op,
    STATE(17), 1,
      sym_mult_expr,
    STATE(20), 1,
      sym_remote_expr,
    STATE(26), 1,
      sym_prefix_expr,
    STATE(31), 1,
      sym_orelse_expr,
    STATE(33), 1,
      sym_add_expr,
    STATE(34), 1,
      sym_andalso_expr,
    STATE(35), 1,
      sym_comp_expr,
    STATE(36), 1,
      sym_list_expr,
    STATE(41), 1,
      sym_equal_exclam_expr,
    STATE(50), 1,
      sym_catch_expr,
    STATE(61), 1,
      sym_exprs,
    STATE(74), 1,
      sym_function_clause_exprs_trailing_comma,
    ACTIONS(21), 2,
      sym_variable,
      sym_integer,
    STATE(8), 2,
      sym_string,
      aux_sym_strings_repeat1,
    STATE(27), 2,
      sym__other_expr,
      sym_function_call_block,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    STATE(21), 4,
      sym__primary_expr,
      sym_list_block,
      sym__atomic,
      sym_strings,
  [252] = 22,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(11), 1,
      sym_atom,
    ACTIONS(15), 1,
      anon_sym_LBRACK,
    ACTIONS(19), 1,
      anon_sym_catch,
    ACTIONS(23), 1,
      anon_sym_DQUOTE,
    STATE(2), 1,
      sym_function_call_open,
    STATE(6), 1,
      sym_catch_op,
    STATE(17), 1,
      sym_mult_expr,
    STATE(20), 1,
      sym_remote_expr,
    STATE(26), 1,
      sym_prefix_expr,
    STATE(31), 1,
      sym_orelse_expr,
    STATE(33), 1,
      sym_add_expr,
    STATE(34), 1,
      sym_andalso_expr,
    STATE(35), 1,
      sym_comp_expr,
    STATE(36), 1,
      sym_list_expr,
    STATE(40), 1,
      sym_catch_expr,
    STATE(41), 1,
      sym_equal_exclam_expr,
    ACTIONS(21), 2,
      sym_variable,
      sym_integer,
    STATE(8), 2,
      sym_string,
      aux_sym_strings_repeat1,
    STATE(27), 2,
      sym__other_expr,
      sym_function_call_block,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    STATE(21), 4,
      sym__primary_expr,
      sym_list_block,
      sym__atomic,
      sym_strings,
  [328] = 18,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(15), 1,
      anon_sym_LBRACK,
    ACTIONS(23), 1,
      anon_sym_DQUOTE,
    STATE(2), 1,
      sym_function_call_open,
    STATE(17), 1,
      sym_mult_expr,
    STATE(20), 1,
      sym_remote_expr,
    STATE(26), 1,
      sym_prefix_expr,
    STATE(31), 1,
      sym_orelse_expr,
    STATE(33), 1,
      sym_add_expr,
    STATE(34), 1,
      sym_andalso_expr,
    STATE(35), 1,
      sym_comp_expr,
    STATE(36), 1,
      sym_list_expr,
    STATE(47), 1,
      sym_equal_exclam_expr,
    STATE(8), 2,
      sym_string,
      aux_sym_strings_repeat1,
    STATE(27), 2,
      sym__other_expr,
      sym_function_call_block,
    ACTIONS(21), 3,
      sym_variable,
      sym_atom,
      sym_integer,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    STATE(21), 4,
      sym__primary_expr,
      sym_list_block,
      sym__atomic,
      sym_strings,
  [393] = 18,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(15), 1,
      anon_sym_LBRACK,
    ACTIONS(23), 1,
      anon_sym_DQUOTE,
    STATE(2), 1,
      sym_function_call_open,
    STATE(17), 1,
      sym_mult_expr,
    STATE(20), 1,
      sym_remote_expr,
    STATE(26), 1,
      sym_prefix_expr,
    STATE(31), 1,
      sym_orelse_expr,
    STATE(33), 1,
      sym_add_expr,
    STATE(34), 1,
      sym_andalso_expr,
    STATE(35), 1,
      sym_comp_expr,
    STATE(36), 1,
      sym_list_expr,
    STATE(42), 1,
      sym_equal_exclam_expr,
    STATE(8), 2,
      sym_string,
      aux_sym_strings_repeat1,
    STATE(27), 2,
      sym__other_expr,
      sym_function_call_block,
    ACTIONS(21), 3,
      sym_variable,
      sym_atom,
      sym_integer,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    STATE(21), 4,
      sym__primary_expr,
      sym_list_block,
      sym__atomic,
      sym_strings,
  [458] = 5,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(23), 1,
      anon_sym_DQUOTE,
    STATE(9), 2,
      sym_string,
      aux_sym_strings_repeat1,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(29), 17,
      anon_sym_DOT,
      anon_sym_DASH,
      anon_sym_LPAREN,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
      anon_sym_COLON,
  [494] = 5,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(33), 1,
      anon_sym_DQUOTE,
    STATE(9), 2,
      sym_string,
      aux_sym_strings_repeat1,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(31), 17,
      anon_sym_DOT,
      anon_sym_DASH,
      anon_sym_LPAREN,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
      anon_sym_COLON,
  [530] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(36), 18,
      anon_sym_DOT,
      anon_sym_DASH,
      anon_sym_LPAREN,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
      anon_sym_COLON,
      anon_sym_DQUOTE,
  [560] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(38), 18,
      anon_sym_DOT,
      anon_sym_DASH,
      anon_sym_LPAREN,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
      anon_sym_COLON,
      anon_sym_DQUOTE,
  [590] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(40), 17,
      anon_sym_DOT,
      anon_sym_DASH,
      anon_sym_LPAREN,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
      anon_sym_COLON,
  [619] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(42), 17,
      anon_sym_DOT,
      anon_sym_DASH,
      anon_sym_LPAREN,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
      anon_sym_COLON,
  [648] = 6,
    ACTIONS(3), 1,
      sym__newline,
    STATE(15), 1,
      sym_add_op,
    STATE(16), 1,
      aux_sym_add_expr_repeat1,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(44), 7,
      anon_sym_DOT,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
    ACTIONS(46), 8,
      anon_sym_DASH,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
  [683] = 12,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(15), 1,
      anon_sym_LBRACK,
    ACTIONS(23), 1,
      anon_sym_DQUOTE,
    STATE(2), 1,
      sym_function_call_open,
    STATE(20), 1,
      sym_remote_expr,
    STATE(25), 1,
      sym_mult_expr,
    STATE(26), 1,
      sym_prefix_expr,
    STATE(8), 2,
      sym_string,
      aux_sym_strings_repeat1,
    STATE(27), 2,
      sym__other_expr,
      sym_function_call_block,
    ACTIONS(21), 3,
      sym_variable,
      sym_atom,
      sym_integer,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    STATE(21), 4,
      sym__primary_expr,
      sym_list_block,
      sym__atomic,
      sym_strings,
  [730] = 6,
    ACTIONS(3), 1,
      sym__newline,
    STATE(15), 1,
      sym_add_op,
    STATE(16), 1,
      aux_sym_add_expr_repeat1,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(48), 7,
      anon_sym_DOT,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
    ACTIONS(50), 8,
      anon_sym_DASH,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
  [765] = 6,
    ACTIONS(3), 1,
      sym__newline,
    STATE(14), 1,
      aux_sym_add_expr_repeat1,
    STATE(15), 1,
      sym_add_op,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(53), 7,
      anon_sym_DOT,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
    ACTIONS(46), 8,
      anon_sym_DASH,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
  [800] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(55), 17,
      anon_sym_DOT,
      anon_sym_DASH,
      anon_sym_LPAREN,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
      anon_sym_COLON,
  [829] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(57), 16,
      anon_sym_DOT,
      anon_sym_DASH,
      anon_sym_LPAREN,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
  [857] = 4,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(61), 1,
      anon_sym_LPAREN,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(59), 15,
      anon_sym_DOT,
      anon_sym_DASH,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
  [887] = 4,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(63), 1,
      anon_sym_COLON,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(59), 15,
      anon_sym_DOT,
      anon_sym_DASH,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
  [917] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(65), 15,
      anon_sym_DOT,
      anon_sym_DASH,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
  [944] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(67), 15,
      anon_sym_DOT,
      anon_sym_DASH,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
  [971] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(69), 15,
      anon_sym_DOT,
      anon_sym_DASH,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
  [998] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(48), 15,
      anon_sym_DOT,
      anon_sym_DASH,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
  [1025] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(71), 15,
      anon_sym_DOT,
      anon_sym_DASH,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
  [1052] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(73), 15,
      anon_sym_DOT,
      anon_sym_DASH,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
      anon_sym_PLUS,
      anon_sym_bor,
      anon_sym_bxor,
      anon_sym_bsl,
      anon_sym_bsr,
      anon_sym_or,
      anon_sym_xor,
  [1079] = 10,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(75), 1,
      ts_builtin_sym_end,
    ACTIONS(77), 1,
      sym_atom,
    ACTIONS(80), 1,
      anon_sym_DASH,
    STATE(4), 1,
      sym_function_clause_open,
    STATE(57), 1,
      sym_function_clause_block,
    STATE(79), 1,
      sym_function_clauses,
    STATE(28), 2,
      sym_form,
      aux_sym_source_file_repeat1,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    STATE(85), 5,
      sym__attribute,
      sym_module_attribute,
      sym_export_attribute,
      sym__function_or_macro,
      sym_function_clauses_trailing_semicolon,
  [1118] = 10,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(5), 1,
      sym_atom,
    ACTIONS(7), 1,
      anon_sym_DASH,
    ACTIONS(83), 1,
      ts_builtin_sym_end,
    STATE(4), 1,
      sym_function_clause_open,
    STATE(57), 1,
      sym_function_clause_block,
    STATE(79), 1,
      sym_function_clauses,
    STATE(28), 2,
      sym_form,
      aux_sym_source_file_repeat1,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    STATE(85), 5,
      sym__attribute,
      sym_module_attribute,
      sym_export_attribute,
      sym__function_or_macro,
      sym_function_clauses_trailing_semicolon,
  [1157] = 7,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(15), 1,
      anon_sym_LBRACK,
    ACTIONS(23), 1,
      anon_sym_DQUOTE,
    STATE(8), 2,
      sym_string,
      aux_sym_strings_repeat1,
    ACTIONS(85), 3,
      sym_variable,
      sym_atom,
      sym_integer,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    STATE(19), 4,
      sym__primary_expr,
      sym_list_block,
      sym__atomic,
      sym_strings,
  [1188] = 6,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(89), 1,
      anon_sym_EQ,
    ACTIONS(91), 1,
      anon_sym_BANG,
    STATE(7), 2,
      sym_equal_op,
      sym_exclam_op,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(87), 5,
      anon_sym_DOT,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
  [1215] = 4,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(93), 2,
      anon_sym_catch,
      sym_atom,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(95), 6,
      anon_sym_RPAREN,
      anon_sym_LBRACK,
      anon_sym_COMMA,
      sym_variable,
      sym_integer,
      anon_sym_DQUOTE,
  [1237] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(97), 7,
      anon_sym_DOT,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
  [1256] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(99), 7,
      anon_sym_DOT,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
  [1275] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(101), 7,
      anon_sym_DOT,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
  [1294] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(103), 7,
      anon_sym_DOT,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
      anon_sym_EQ,
      anon_sym_BANG,
  [1313] = 4,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(105), 2,
      anon_sym_catch,
      sym_atom,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(107), 4,
      anon_sym_LBRACK,
      sym_variable,
      sym_integer,
      anon_sym_DQUOTE,
  [1333] = 5,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(111), 1,
      anon_sym_COMMA,
    STATE(38), 1,
      aux_sym_exprs_repeat1,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(109), 4,
      anon_sym_DOT,
      anon_sym_RPAREN,
      anon_sym_RBRACK,
      anon_sym_SEMI,
  [1355] = 4,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(114), 2,
      anon_sym_catch,
      sym_atom,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(116), 4,
      anon_sym_LBRACK,
      sym_variable,
      sym_integer,
      anon_sym_DQUOTE,
  [1375] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(109), 5,
      anon_sym_DOT,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
  [1392] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(118), 5,
      anon_sym_DOT,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
  [1409] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(120), 5,
      anon_sym_DOT,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
  [1426] = 7,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(122), 1,
      sym_atom,
    ACTIONS(124), 1,
      anon_sym_COMMA,
    ACTIONS(126), 1,
      anon_sym_RBRACK,
    STATE(65), 1,
      sym_export_attribute_mfa,
    STATE(68), 1,
      sym_export_attribute_mfas,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1451] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(128), 5,
      anon_sym_LBRACK,
      sym_variable,
      sym_atom,
      sym_integer,
      anon_sym_DQUOTE,
  [1468] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(130), 5,
      anon_sym_LBRACK,
      sym_variable,
      sym_atom,
      sym_integer,
      anon_sym_DQUOTE,
  [1485] = 5,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(134), 1,
      anon_sym_COMMA,
    STATE(38), 1,
      aux_sym_exprs_repeat1,
    ACTIONS(132), 3,
      anon_sym_DOT,
      anon_sym_RPAREN,
      anon_sym_SEMI,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1506] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(137), 5,
      anon_sym_DOT,
      anon_sym_RPAREN,
      anon_sym_COMMA,
      anon_sym_RBRACK,
      anon_sym_SEMI,
  [1523] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(139), 5,
      anon_sym_LBRACK,
      sym_variable,
      sym_atom,
      sym_integer,
      anon_sym_DQUOTE,
  [1540] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
    ACTIONS(141), 5,
      anon_sym_LBRACK,
      sym_variable,
      sym_atom,
      sym_integer,
      anon_sym_DQUOTE,
  [1557] = 5,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(145), 1,
      anon_sym_COMMA,
    STATE(46), 1,
      aux_sym_exprs_repeat1,
    ACTIONS(143), 3,
      anon_sym_DOT,
      anon_sym_RPAREN,
      anon_sym_SEMI,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1578] = 4,
    ACTIONS(148), 1,
      anon_sym_DQUOTE,
    STATE(51), 1,
      aux_sym_string_repeat1,
    ACTIONS(150), 2,
      aux_sym_string_token1,
      sym__escape_sequence,
    ACTIONS(3), 5,
      sym_comment,
      sym_line_comment,
      sym__newline,
      sym_multiple_newlines,
      sym__spaces,
  [1596] = 4,
    ACTIONS(153), 1,
      anon_sym_DQUOTE,
    STATE(53), 1,
      aux_sym_string_repeat1,
    ACTIONS(155), 2,
      aux_sym_string_token1,
      sym__escape_sequence,
    ACTIONS(3), 5,
      sym_comment,
      sym_line_comment,
      sym__newline,
      sym_multiple_newlines,
      sym__spaces,
  [1614] = 4,
    ACTIONS(157), 1,
      anon_sym_DQUOTE,
    STATE(51), 1,
      aux_sym_string_repeat1,
    ACTIONS(159), 2,
      aux_sym_string_token1,
      sym__escape_sequence,
    ACTIONS(3), 5,
      sym_comment,
      sym_line_comment,
      sym__newline,
      sym_multiple_newlines,
      sym__spaces,
  [1632] = 5,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(5), 1,
      sym_atom,
    STATE(4), 1,
      sym_function_clause_open,
    STATE(70), 1,
      sym_function_clause_block,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1651] = 5,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(161), 1,
      anon_sym_DOT,
    ACTIONS(163), 1,
      anon_sym_SEMI,
    STATE(64), 1,
      aux_sym_function_clauses_repeat1,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1670] = 5,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(166), 1,
      anon_sym_COMMA,
    ACTIONS(169), 1,
      anon_sym_RBRACK,
    STATE(62), 1,
      aux_sym_exprs_repeat1,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1689] = 5,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(171), 1,
      anon_sym_DOT,
    ACTIONS(173), 1,
      anon_sym_SEMI,
    STATE(55), 1,
      aux_sym_function_clauses_repeat1,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1708] = 5,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(176), 1,
      anon_sym_COMMA,
    ACTIONS(179), 1,
      anon_sym_RBRACK,
    STATE(59), 1,
      aux_sym_export_attribute_mfas_repeat1,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1727] = 5,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(181), 1,
      anon_sym_COMMA,
    ACTIONS(184), 1,
      anon_sym_RBRACK,
    STATE(59), 1,
      aux_sym_export_attribute_mfas_repeat1,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1746] = 5,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(186), 1,
      anon_sym_DASH_GT,
    ACTIONS(188), 1,
      anon_sym_when,
    STATE(103), 1,
      sym_clause_guard,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1765] = 4,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(192), 1,
      anon_sym_COMMA,
    ACTIONS(190), 2,
      anon_sym_DOT,
      anon_sym_SEMI,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1782] = 5,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(194), 1,
      anon_sym_COMMA,
    ACTIONS(197), 1,
      anon_sym_RBRACK,
    STATE(38), 1,
      aux_sym_exprs_repeat1,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1801] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(199), 3,
      ts_builtin_sym_end,
      anon_sym_DASH,
      sym_atom,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1816] = 5,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(201), 1,
      anon_sym_DOT,
    ACTIONS(203), 1,
      anon_sym_SEMI,
    STATE(64), 1,
      aux_sym_function_clauses_repeat1,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1835] = 5,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(206), 1,
      anon_sym_COMMA,
    ACTIONS(209), 1,
      anon_sym_RBRACK,
    STATE(58), 1,
      aux_sym_export_attribute_mfas_repeat1,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1854] = 4,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(211), 1,
      anon_sym_RPAREN,
    ACTIONS(213), 1,
      anon_sym_COMMA,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1870] = 4,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(215), 1,
      anon_sym_module,
    ACTIONS(217), 1,
      anon_sym_export,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1886] = 4,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(219), 1,
      anon_sym_COMMA,
    ACTIONS(221), 1,
      anon_sym_RBRACK,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1902] = 4,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(223), 1,
      anon_sym_LPAREN,
    STATE(60), 1,
      sym_pat_argument_list,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1918] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(201), 2,
      anon_sym_DOT,
      anon_sym_SEMI,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1932] = 4,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(225), 1,
      anon_sym_LBRACK,
    STATE(80), 1,
      sym_export_attribute_block,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1948] = 4,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(122), 1,
      sym_atom,
    STATE(76), 1,
      sym_export_attribute_mfa,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1964] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(227), 2,
      anon_sym_COMMA,
      anon_sym_RBRACK,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1978] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(229), 2,
      anon_sym_DOT,
      anon_sym_SEMI,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [1992] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(231), 2,
      anon_sym_DASH_GT,
      anon_sym_when,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2006] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(184), 2,
      anon_sym_COMMA,
      anon_sym_RBRACK,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2020] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(233), 2,
      anon_sym_DOT,
      anon_sym_SEMI,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2034] = 4,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(235), 1,
      anon_sym_COMMA,
    ACTIONS(237), 1,
      anon_sym_RBRACK,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2050] = 4,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(239), 1,
      anon_sym_DOT,
    ACTIONS(241), 1,
      anon_sym_SEMI,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2066] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(243), 1,
      anon_sym_RPAREN,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2079] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(245), 1,
      anon_sym_LPAREN,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2092] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(247), 1,
      anon_sym_RPAREN,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2105] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(249), 1,
      anon_sym_LPAREN,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2118] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(251), 1,
      anon_sym_RPAREN,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2131] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(253), 1,
      anon_sym_DOT,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2144] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(255), 1,
      anon_sym_DOT,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2157] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(221), 1,
      anon_sym_RBRACK,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2170] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(257), 1,
      anon_sym_RPAREN,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2183] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(259), 1,
      anon_sym_SLASH,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2196] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(261), 1,
      anon_sym_RBRACK,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2209] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(263), 1,
      anon_sym_DASH_GT,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2222] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(265), 1,
      anon_sym_DOT,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2235] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(267), 1,
      ts_builtin_sym_end,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2248] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(269), 1,
      anon_sym_DOT,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2261] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(271), 1,
      anon_sym_RPAREN,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2274] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(273), 1,
      sym_integer,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2287] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(275), 1,
      anon_sym_RBRACK,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2300] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(277), 1,
      anon_sym_RPAREN,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2313] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(211), 1,
      anon_sym_RPAREN,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2326] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(279), 1,
      sym_atom,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2339] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(281), 1,
      anon_sym_RPAREN,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2352] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(237), 1,
      anon_sym_RBRACK,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
  [2365] = 3,
    ACTIONS(3), 1,
      sym__newline,
    ACTIONS(283), 1,
      anon_sym_DASH_GT,
    ACTIONS(9), 4,
      sym_comment,
      sym_line_comment,
      sym_multiple_newlines,
      sym__spaces,
};

static uint32_t ts_small_parse_table_map[] = {
  [SMALL_STATE(2)] = 0,
  [SMALL_STATE(3)] = 85,
  [SMALL_STATE(4)] = 170,
  [SMALL_STATE(5)] = 252,
  [SMALL_STATE(6)] = 328,
  [SMALL_STATE(7)] = 393,
  [SMALL_STATE(8)] = 458,
  [SMALL_STATE(9)] = 494,
  [SMALL_STATE(10)] = 530,
  [SMALL_STATE(11)] = 560,
  [SMALL_STATE(12)] = 590,
  [SMALL_STATE(13)] = 619,
  [SMALL_STATE(14)] = 648,
  [SMALL_STATE(15)] = 683,
  [SMALL_STATE(16)] = 730,
  [SMALL_STATE(17)] = 765,
  [SMALL_STATE(18)] = 800,
  [SMALL_STATE(19)] = 829,
  [SMALL_STATE(20)] = 857,
  [SMALL_STATE(21)] = 887,
  [SMALL_STATE(22)] = 917,
  [SMALL_STATE(23)] = 944,
  [SMALL_STATE(24)] = 971,
  [SMALL_STATE(25)] = 998,
  [SMALL_STATE(26)] = 1025,
  [SMALL_STATE(27)] = 1052,
  [SMALL_STATE(28)] = 1079,
  [SMALL_STATE(29)] = 1118,
  [SMALL_STATE(30)] = 1157,
  [SMALL_STATE(31)] = 1188,
  [SMALL_STATE(32)] = 1215,
  [SMALL_STATE(33)] = 1237,
  [SMALL_STATE(34)] = 1256,
  [SMALL_STATE(35)] = 1275,
  [SMALL_STATE(36)] = 1294,
  [SMALL_STATE(37)] = 1313,
  [SMALL_STATE(38)] = 1333,
  [SMALL_STATE(39)] = 1355,
  [SMALL_STATE(40)] = 1375,
  [SMALL_STATE(41)] = 1392,
  [SMALL_STATE(42)] = 1409,
  [SMALL_STATE(43)] = 1426,
  [SMALL_STATE(44)] = 1451,
  [SMALL_STATE(45)] = 1468,
  [SMALL_STATE(46)] = 1485,
  [SMALL_STATE(47)] = 1506,
  [SMALL_STATE(48)] = 1523,
  [SMALL_STATE(49)] = 1540,
  [SMALL_STATE(50)] = 1557,
  [SMALL_STATE(51)] = 1578,
  [SMALL_STATE(52)] = 1596,
  [SMALL_STATE(53)] = 1614,
  [SMALL_STATE(54)] = 1632,
  [SMALL_STATE(55)] = 1651,
  [SMALL_STATE(56)] = 1670,
  [SMALL_STATE(57)] = 1689,
  [SMALL_STATE(58)] = 1708,
  [SMALL_STATE(59)] = 1727,
  [SMALL_STATE(60)] = 1746,
  [SMALL_STATE(61)] = 1765,
  [SMALL_STATE(62)] = 1782,
  [SMALL_STATE(63)] = 1801,
  [SMALL_STATE(64)] = 1816,
  [SMALL_STATE(65)] = 1835,
  [SMALL_STATE(66)] = 1854,
  [SMALL_STATE(67)] = 1870,
  [SMALL_STATE(68)] = 1886,
  [SMALL_STATE(69)] = 1902,
  [SMALL_STATE(70)] = 1918,
  [SMALL_STATE(71)] = 1932,
  [SMALL_STATE(72)] = 1948,
  [SMALL_STATE(73)] = 1964,
  [SMALL_STATE(74)] = 1978,
  [SMALL_STATE(75)] = 1992,
  [SMALL_STATE(76)] = 2006,
  [SMALL_STATE(77)] = 2020,
  [SMALL_STATE(78)] = 2034,
  [SMALL_STATE(79)] = 2050,
  [SMALL_STATE(80)] = 2066,
  [SMALL_STATE(81)] = 2079,
  [SMALL_STATE(82)] = 2092,
  [SMALL_STATE(83)] = 2105,
  [SMALL_STATE(84)] = 2118,
  [SMALL_STATE(85)] = 2131,
  [SMALL_STATE(86)] = 2144,
  [SMALL_STATE(87)] = 2157,
  [SMALL_STATE(88)] = 2170,
  [SMALL_STATE(89)] = 2183,
  [SMALL_STATE(90)] = 2196,
  [SMALL_STATE(91)] = 2209,
  [SMALL_STATE(92)] = 2222,
  [SMALL_STATE(93)] = 2235,
  [SMALL_STATE(94)] = 2248,
  [SMALL_STATE(95)] = 2261,
  [SMALL_STATE(96)] = 2274,
  [SMALL_STATE(97)] = 2287,
  [SMALL_STATE(98)] = 2300,
  [SMALL_STATE(99)] = 2313,
  [SMALL_STATE(100)] = 2326,
  [SMALL_STATE(101)] = 2339,
  [SMALL_STATE(102)] = 2352,
  [SMALL_STATE(103)] = 2365,
};

static TSParseActionEntry ts_parse_actions[] = {
  [0] = {.entry = {.count = 0, .reusable = false}},
  [1] = {.entry = {.count = 1, .reusable = false}}, RECOVER(),
  [3] = {.entry = {.count = 1, .reusable = false}}, SHIFT_EXTRA(),
  [5] = {.entry = {.count = 1, .reusable = true}}, SHIFT(69),
  [7] = {.entry = {.count = 1, .reusable = true}}, SHIFT(67),
  [9] = {.entry = {.count = 1, .reusable = true}}, SHIFT_EXTRA(),
  [11] = {.entry = {.count = 1, .reusable = false}}, SHIFT(21),
  [13] = {.entry = {.count = 1, .reusable = true}}, SHIFT(23),
  [15] = {.entry = {.count = 1, .reusable = true}}, SHIFT(3),
  [17] = {.entry = {.count = 1, .reusable = true}}, SHIFT(99),
  [19] = {.entry = {.count = 1, .reusable = false}}, SHIFT(44),
  [21] = {.entry = {.count = 1, .reusable = true}}, SHIFT(21),
  [23] = {.entry = {.count = 1, .reusable = true}}, SHIFT(52),
  [25] = {.entry = {.count = 1, .reusable = true}}, SHIFT(102),
  [27] = {.entry = {.count = 1, .reusable = true}}, SHIFT(13),
  [29] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_strings, 1),
  [31] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_strings_repeat1, 2),
  [33] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_strings_repeat1, 2), SHIFT_REPEAT(52),
  [36] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_string, 3),
  [38] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_string, 2),
  [40] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_list_block, 3),
  [42] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_list_block, 2),
  [44] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_add_expr, 2),
  [46] = {.entry = {.count = 1, .reusable = true}}, SHIFT(45),
  [48] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_add_expr_repeat1, 2),
  [50] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_add_expr_repeat1, 2), SHIFT_REPEAT(45),
  [53] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_add_expr, 1),
  [55] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_list_block, 4),
  [57] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_remote_expr, 3),
  [59] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__other_expr, 1),
  [61] = {.entry = {.count = 1, .reusable = true}}, SHIFT(32),
  [63] = {.entry = {.count = 1, .reusable = true}}, SHIFT(30),
  [65] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_call_block, 3),
  [67] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_call_block, 2),
  [69] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_call_block, 4),
  [71] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_mult_expr, 1),
  [73] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_prefix_expr, 1),
  [75] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2),
  [77] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2), SHIFT_REPEAT(69),
  [80] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2), SHIFT_REPEAT(67),
  [83] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_source_file, 1),
  [85] = {.entry = {.count = 1, .reusable = true}}, SHIFT(19),
  [87] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_equal_exclam_expr, 1),
  [89] = {.entry = {.count = 1, .reusable = true}}, SHIFT(48),
  [91] = {.entry = {.count = 1, .reusable = true}}, SHIFT(49),
  [93] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_function_call_open, 2),
  [95] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_call_open, 2),
  [97] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_list_expr, 1),
  [99] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_orelse_expr, 1),
  [101] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_andalso_expr, 1),
  [103] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_comp_expr, 1),
  [105] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_function_clause_open, 3),
  [107] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_clause_open, 3),
  [109] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_exprs_repeat1, 2),
  [111] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_exprs_repeat1, 2), SHIFT_REPEAT(5),
  [114] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_function_clause_open, 4),
  [116] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_clause_open, 4),
  [118] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_catch_expr, 1),
  [120] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_equal_exclam_expr, 3),
  [122] = {.entry = {.count = 1, .reusable = true}}, SHIFT(89),
  [124] = {.entry = {.count = 1, .reusable = true}}, SHIFT(87),
  [126] = {.entry = {.count = 1, .reusable = true}}, SHIFT(88),
  [128] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_catch_op, 1),
  [130] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_add_op, 1),
  [132] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_exprs, 2),
  [134] = {.entry = {.count = 2, .reusable = true}}, REDUCE(sym_exprs, 2), SHIFT(5),
  [137] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_catch_expr, 2),
  [139] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_equal_op, 1),
  [141] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_exclam_op, 1),
  [143] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_exprs, 1),
  [145] = {.entry = {.count = 2, .reusable = true}}, REDUCE(sym_exprs, 1), SHIFT(5),
  [148] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_string_repeat1, 2),
  [150] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_string_repeat1, 2), SHIFT_REPEAT(51),
  [153] = {.entry = {.count = 1, .reusable = true}}, SHIFT(11),
  [155] = {.entry = {.count = 1, .reusable = true}}, SHIFT(53),
  [157] = {.entry = {.count = 1, .reusable = true}}, SHIFT(10),
  [159] = {.entry = {.count = 1, .reusable = true}}, SHIFT(51),
  [161] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_clauses, 2),
  [163] = {.entry = {.count = 2, .reusable = true}}, REDUCE(sym_function_clauses, 2), SHIFT(54),
  [166] = {.entry = {.count = 2, .reusable = true}}, REDUCE(sym_list_elements, 1), SHIFT(5),
  [169] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_list_elements, 1),
  [171] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_clauses, 1),
  [173] = {.entry = {.count = 2, .reusable = true}}, REDUCE(sym_function_clauses, 1), SHIFT(54),
  [176] = {.entry = {.count = 2, .reusable = true}}, REDUCE(sym_export_attribute_mfas, 2), SHIFT(72),
  [179] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_export_attribute_mfas, 2),
  [181] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_export_attribute_mfas_repeat1, 2), SHIFT_REPEAT(72),
  [184] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_export_attribute_mfas_repeat1, 2),
  [186] = {.entry = {.count = 1, .reusable = true}}, SHIFT(37),
  [188] = {.entry = {.count = 1, .reusable = true}}, SHIFT(91),
  [190] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_clause_exprs_trailing_comma, 1, .production_id = 1),
  [192] = {.entry = {.count = 1, .reusable = true}}, SHIFT(77),
  [194] = {.entry = {.count = 2, .reusable = true}}, REDUCE(sym_list_elements, 2), SHIFT(5),
  [197] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_list_elements, 2),
  [199] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_form, 2),
  [201] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_function_clauses_repeat1, 2),
  [203] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_function_clauses_repeat1, 2), SHIFT_REPEAT(54),
  [206] = {.entry = {.count = 2, .reusable = true}}, REDUCE(sym_export_attribute_mfas, 1), SHIFT(72),
  [209] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_export_attribute_mfas, 1),
  [211] = {.entry = {.count = 1, .reusable = true}}, SHIFT(22),
  [213] = {.entry = {.count = 1, .reusable = true}}, SHIFT(84),
  [215] = {.entry = {.count = 1, .reusable = true}}, SHIFT(81),
  [217] = {.entry = {.count = 1, .reusable = true}}, SHIFT(83),
  [219] = {.entry = {.count = 1, .reusable = true}}, SHIFT(97),
  [221] = {.entry = {.count = 1, .reusable = true}}, SHIFT(95),
  [223] = {.entry = {.count = 1, .reusable = true}}, SHIFT(82),
  [225] = {.entry = {.count = 1, .reusable = true}}, SHIFT(43),
  [227] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_export_attribute_mfa, 3),
  [229] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_clause_block, 2),
  [231] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_pat_argument_list, 2),
  [233] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_clause_exprs_trailing_comma, 2, .production_id = 1),
  [235] = {.entry = {.count = 1, .reusable = true}}, SHIFT(90),
  [237] = {.entry = {.count = 1, .reusable = true}}, SHIFT(12),
  [239] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_clauses_trailing_semicolon, 1),
  [241] = {.entry = {.count = 1, .reusable = true}}, SHIFT(94),
  [243] = {.entry = {.count = 1, .reusable = true}}, SHIFT(92),
  [245] = {.entry = {.count = 1, .reusable = true}}, SHIFT(100),
  [247] = {.entry = {.count = 1, .reusable = true}}, SHIFT(75),
  [249] = {.entry = {.count = 1, .reusable = true}}, SHIFT(71),
  [251] = {.entry = {.count = 1, .reusable = true}}, SHIFT(24),
  [253] = {.entry = {.count = 1, .reusable = true}}, SHIFT(63),
  [255] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_module_attribute, 5),
  [257] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_export_attribute_block, 2),
  [259] = {.entry = {.count = 1, .reusable = true}}, SHIFT(96),
  [261] = {.entry = {.count = 1, .reusable = true}}, SHIFT(18),
  [263] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_clause_guard, 1),
  [265] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_export_attribute, 5),
  [267] = {.entry = {.count = 1, .reusable = true}},  ACCEPT_INPUT(),
  [269] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_clauses_trailing_semicolon, 2),
  [271] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_export_attribute_block, 3),
  [273] = {.entry = {.count = 1, .reusable = true}}, SHIFT(73),
  [275] = {.entry = {.count = 1, .reusable = true}}, SHIFT(101),
  [277] = {.entry = {.count = 1, .reusable = true}}, SHIFT(86),
  [279] = {.entry = {.count = 1, .reusable = true}}, SHIFT(98),
  [281] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_export_attribute_block, 4),
  [283] = {.entry = {.count = 1, .reusable = true}}, SHIFT(39),
};

#ifdef __cplusplus
extern "C" {
#endif
#ifdef _WIN32
#define extern __declspec(dllexport)
#endif

extern const TSLanguage *tree_sitter_erlang(void) {
  static TSLanguage language = {
    .version = LANGUAGE_VERSION,
    .symbol_count = SYMBOL_COUNT,
    .alias_count = ALIAS_COUNT,
    .token_count = TOKEN_COUNT,
    .external_token_count = EXTERNAL_TOKEN_COUNT,
    .symbol_names = ts_symbol_names,
    .symbol_metadata = ts_symbol_metadata,
    .parse_table = (const uint16_t *)ts_parse_table,
    .parse_actions = ts_parse_actions,
    .lex_modes = ts_lex_modes,
    .alias_sequences = (const TSSymbol *)ts_alias_sequences,
    .max_alias_sequence_length = MAX_ALIAS_SEQUENCE_LENGTH,
    .lex_fn = ts_lex,
    .keyword_lex_fn = ts_lex_keywords,
    .keyword_capture_token = sym_atom,
    .field_count = FIELD_COUNT,
    .large_state_count = LARGE_STATE_COUNT,
    .small_parse_table = (const uint16_t *)ts_small_parse_table,
    .small_parse_table_map = (const uint32_t *)ts_small_parse_table_map,
    .public_symbol_map = ts_symbol_map,
    .alias_map = ts_non_terminal_alias_map,
    .state_count = STATE_COUNT,
  };
  return &language;
}
#ifdef __cplusplus
}
#endif
